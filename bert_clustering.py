...
for NUM_NEIGHBORS in range(2,10):
    try:
        ...
        umap_model = UMAP(n_neighbors=NUM_NEIGHBORS, n_components=5, min_dist=0.0, metric='cosine')
        ...
        for nc in range(len(topic_docs)-1):
            with open(os.path.join(cluster_path, f'topic_{nc}_of_{len(topic_docs)-1}'), 'w') as f:
        ...
        fig1.update_layout(title=f"Topic Distances {len(topic_docs)-1} clusters, NUM_NEIGHBORS {NUM_NEIGHBORS}")
        fig1.write_html(os.path.join(cluster_path, f"topic_clusters_distance_num_topics_{len(topic_docs)-1}.html"))
        # fig1.show()

        fig2 = topic_model.visualize_barchart(top_n_topics=len(topic_docs)-1)
        fig2.write_html(os.path.join(cluster_path, f"topic_barchart_wordscores_num_topics_{len(topic_docs)-1}.html"))
        fig2.write_image(os.path.join(cluster_path, f"topic_barchart_wordscores_num_topics_{len(topic_docs)-1}.png"))
        # fig2.show()

        # Run the visualization with the original embeddings
        fig3 = topic_model.visualize_documents(docs, embeddings=embeddings)
        fig3.update_layout(title=f"Clusters with original embeddings {len(topic_docs)-1} clusters, NUM_NEIGHBORS {NUM_NEIGHBORS}")
        fig3.write_html(os.path.join(cluster_path, f"topic_clusters_original_embeddings_num_topics_{len(topic_docs)-1}.html"))
        fig3.write_image(os.path.join(cluster_path, f"topic_clusters_original_embeddings_num_topics_{len(topic_docs)-1}.png"))
        # fig3.show()

        # Reduce dimensionality of embeddings, this step is optional but much faster to perform iteratively:
        reduced_embeddings = umap_model.fit_transform(embeddings)
        fig4 = topic_model.visualize_documents(docs, reduced_embeddings=reduced_embeddings)
        fig4.update_layout(title=f"Clusters with UMAP reduced embeddings {len(topic_docs)-1} clusters, NUM_NEIGHBORS {NUM_NEIGHBORS}")
        fig4.write_html(os.path.join(cluster_path, f"topic_clusters_UMAP_reduced_embeddings_num_topics_{len(topic_docs)-1}.html"))
        # fig4.show()
