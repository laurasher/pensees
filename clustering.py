...
for NUM_CLUSTERS in range(4,15):
    for K_MEANS_RANDOM_STATE in range(5):
        cluster_path = os.path.jon("kmeans_clusters", f"num_clusters_{NUM_CLUSTERS}_kmeans_random_state_{K_MEANS_RANDOM_STATE}")
        if not os.path.exists(cluster_path):
            os.mkdir(cluster_path)

        k = NUM_CLUSTERS # Tune this parameter through domain knowledge or some kind of ekbow fitting
        km = KMeans(n_clusters=k, init='k-means++', max_iter=100, n_init=5,verbose=1, random_state=K_MEANS_RANDOM_STATE)
        km.fit(tfs)
        ...
        plt.savefig(os.path.join(cluster_path, f"HISTOGRAM num_clusters_{NUM_CLUSTERS}_kmeans_random_state_{K_MEANS_RANDOM_STATE}.png"))
        # plt.show()
        ...
        for i in set(km.labels_):
            current_cluster_p_raw = [raw_document_list[x] for x in np.where(km.labels_ == i)[0]]
        ...
        for key in cluster_assignments_dict.keys():
            ...
            current_tf_idfs = dict(zip(current_tfidf.get_feature_names_out(), current_tfidf.idf_))
        ...
        for nc in range(len(cluster_themes_dict)):
            with open(os.path.join(cluster_path, f'cluster_{nc}_of_{NUM_CLUSTERS}'), 'w') as f:
                print(f'Cluster {nc} key words: {[x[0] for x in cluster_themes_dict[nc]]}')
                print(f'Cluster {nc} contains {len(cluster_assignments_dict[nc])} pensées')

                f.write(f'Cluster {nc} key words: {[x[0] for x in cluster_themes_dict[nc]]}\n')
                f.write(f'Cluster {nc} contains {len(cluster_assignments_dict[nc])} pensées\n')
                f.write("\n-------------------------------------------------------------------------------\n")
        ...
        ax = plt.axes()
        plt.title(f"num_clusters_{NUM_CLUSTERS}_kmeans_random_state_{K_MEANS_RANDOM_STATE}")
        plt.scatter(tfs_embedded[:, 0], tfs_embedded[:, 1], marker = "x", c = km.labels_)
        plt.savefig(os.path.join(cluster_path, f"CLUSTER num_clusters_{NUM_CLUSTERS}_kmeans_random_state_{K_MEANS_RANDOM_STATE}.png"))
        # plt.show()
